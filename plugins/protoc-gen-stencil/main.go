...
import (
	...
	"github.com/gofunct/mamba/plugins"
...
func main() {
	...
	if singlePackageMode {
		...
		protofunc.SetRegistry(registry)
	...
	for _, file := range g.Request.GetProtoFile() {
		if all {
			...
			encoder := plugins.NewGenericTemplateBasedEncoder(templateDir, file, debug, destinationDir)
		...
		for _, service := range file.GetService() {
			encoder := plugins.NewGenericServiceTemplateBasedEncoder(templateDir, service, file, debug, destinationDir)
			for _, tmpl := range encoder.Files() {
				concatOrAppend(tmpl)
			}
		}
